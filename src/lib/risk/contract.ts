...
export function assessDangerousFunctions(contract: {
  ...
  if (contract.hasMintFunction) {
    score += 10;
    warnings.push({
      code: 'MINT_FUNCTION',
      severity: 'high',
      message: 'Token supply can be increased',
    });
  }
  ...
  if (contract.hasBlacklistFunction) {
    score += 5;
    warnings.push({
      code: 'BLACKLIST_FUNCTION',
      severity: 'medium',
      message: 'Addresses can be blacklisted',
    });
  }
  ...
  if (contract.hasProxy) {
    ...
    warnings.push({
      code: 'PROXY_CONTRACT',
      severity: 'high',
      message: 'Contract can be upgraded',
    });
  }

  if (contract.maxTaxPercent > 10) {
    score += 7;
    warnings.push({
      code: 'HIGH_TAX',
      severity: 'high',
      message: `Max tax: ${contract.maxTaxPercent}%`,
    });
  }

  return { score: Math.min(score, 30), warnings };
}
