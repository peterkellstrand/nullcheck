export const getTrendingTokensTool = {
    ...
    description: 'Find tokens currently trending across DEXes, ranked by volume and activity. ' +
...
export async function handleGetTrendingTokens(client, args) {
    const result = await client.get('/api/tokens', { chain: args.chain, limit: args.limit || 20 });
    const tokens = result.tokens || [];
    if (tokens.length === 0) {
        return { content: [{ type: 'text', text: 'No trending tokens found for the specified filters.' }] };
    }
...
//# sourceMappingURL=get-trending-tokens.js.map