export const getWhaleActivityTool = {
    name: 'get_whale_activity',
    description: 'Track large wallet transactions (>$10k or >1% of supply) for a token over the past 24 hours. ' +
...
export async function handleGetWhaleActivity(client, args) {
    const data = await client.get(`/api/whale/activity/${args.chain}/${args.address}`);
    const sentiment = data.netFlow24h > 0 ? 'BULLISH (net buying)' : data.netFlow24h < 0 ? 'BEARISH (net selling)' : 'NEUTRAL';
    const txLines = (data.recentTransactions || []).slice(0, 5).map((tx) => `  ${tx.type.toUpperCase()} $${formatNum(tx.valueUsd)} by ${tx.walletAddress.slice(0, 8)}...${tx.walletAddress.slice(-4)}`);
    const text = [
...
//# sourceMappingURL=get-whale-activity.js.map