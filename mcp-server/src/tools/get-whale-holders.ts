...
import type { TokenHolder } from '../utils/types.js';

export const getWhaleHoldersTool: Tool = {
  name: 'get_whale_holders',
  description:
    'Get the largest holders of a token and their percentage of total supply. ' +
...
export async function handleGetWhaleHolders(
  client: NullcheckClient,
  args: { chain: string; address: string }
) {
  const data = await client.get<{ holders: TokenHolder[]; limit: number; total: number }>(
    `/api/whale/holders/${args.chain}/${args.address}`
  );
