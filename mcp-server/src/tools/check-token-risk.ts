...
import type { RiskScore } from '../utils/types.js';

export const checkTokenRiskTool: Tool = {
  name: 'check_token_risk',
  description:
    'Analyze a DeFi token for honeypot traps, rug pull risk, contract vulnerabilities, and liquidity issues. ' +
...
export async function handleCheckTokenRisk(
  client: NullcheckClient,
  args: { chain: string; address: string; force?: boolean }
) {
  const data = await client.post<RiskScore>(
    `/api/risk/${args.chain}/${args.address}`,
