...
export const searchTokensTool: Tool = {
  name: 'search_tokens',
  description:
    'Find a token by name, symbol, or contract address across Ethereum, Base, and Solana. ' +
...
export async function handleSearchTokens(
  ...
  args: { query: string; chain?: string; limit?: number }
) {
  const result = await client.get<{ results: Token[]; count: number }>('/api/search', {
    q: args.query,
    chain: args.chain,
    limit: args.limit || 10,
  });
  ...
  if (tokens.length === 0) {
    return { content: [{ type: 'text' as const, text: `No tokens found matching "${args.query}".` }] };
  }

  const lines = tokens.map((t, i) =>
    `${i + 1}. ${t.symbol} (${t.name}) on ${t.chainId}\n   Address: ${t.address}`
  );
