...
export const getTokenDetailsTool: Tool = {
  ...
  description:
    'Get comprehensive data for a specific token: current price, 1h/24h price change, trading volume, ' +
...
export async function handleGetTokenDetails(
  client: NullcheckClient,
  args: { chain: string; address: string }
) {
  const data = await client.get<TokenWithMetrics>(
    `/api/token/${args.chain}/${args.address}`
  );
